...
function createEditProductPage() {
  ...
  (async function () {
    try {
      ...
      img.style = `background: url('http://localhost:1337${json.image.formats.large.url}') center no-repeat; background-size: cover; width: 100%; height: 40rem;`;
  ...
  form.addEventListener("submit", (event) =>
    submitForm(name, price, description, featured, imgFile, event)
  );
...
function submitForm(name, price, description, featured, img, event) {
  ...
  if (img.files.length > 0 && notCorrectImage) {
    messageErrorImg.innerHTML = displayMessage(
      "message-error",
      "Must be a JPG, JPEG or PNG image, and it cannot be more than 200MB."
    );
  } else {
  ...
  if (nameValue.length < 1 || nameValue.length > 10) {
    messageErrorName.innerHTML = displayMessage(
      "message-error",
      "Cannot be blank or more than 10 characters."
    );
  } else {
  ...
  if (priceValue <= 0 || isNaN(priceValue)) {
    messageErrorPrice.innerHTML = displayMessage(
      "message-error",
      "Cannot be 0 or blank, and must be a number."
    );
  } else {
  ...
  // description error
  const messageErrorDescription = document.querySelector(
    ".message-error-description"
  );
  if (descriptionValue.length < 10) {
    messageErrorDescription.innerHTML = displayMessage(
      "message-error",
      "Cannot be blank or less than 10 characters."
    );
  } else {
...
async function updateProduct(
  ...
  try {
    ...
    if (json.updated_at) {
      ...
      bottomMessageContainer.innerHTML = displayMessage(
        "message-success",
        `The product was updated successfully. <br /> <br /> To view the live product page, go <a href="product-details.html?id=${json.id}" class="link">here</a>. <br /> <br /> To edit the product again <a href="account-edit-product.html?id=${json.id}" class="link">refresh</a> the page.`
      );
    }
